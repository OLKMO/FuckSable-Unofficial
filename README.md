...
| `entity-lookup-remove-guard` | 拦截 `PersistentEntitySectionManager.stopTracking` 中的 `EntityLookup.remove` 调用，捕获 `Int2ObjectLinkedOpenHashMap.fixPointers` 抛出的 `ArrayIndexOutOfBoundsException`（index -1），防止 Sable 的 SubLevel 实体管理破坏 map 内部链表状态后导致整个 tick 崩溃。属治标修复，单次移除失败会被跳过并打 WARN 日志。/ Intercepts `EntityLookup.remove` inside `PersistentEntitySectionManager.stopTracking`, catches `ArrayIndexOutOfBoundsException` from `Int2ObjectLinkedOpenHashMap.fixPointers` (index -1) when Sable's SubLevel entity management corrupts the map's internal linked-list state. Prevents a single entity removal failure from crashing the entire tick. Symptomatic fix — failed removals are skipped with a WARN log. |
